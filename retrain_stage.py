...
parser.add_argument('--model', type=str, default='optfusion_train', help='model: optfusion_train')
...
parser.add_argument('--mlp_dim', type=int, default=2000, help='dimension for single layer')
...
parser.add_argument('--load', type=str, default='avazu_2_optfusion_search_7935', help='load model arch')
...
log_name = f'{args.dataset}_{args.model}_{current_date}_{"_".join(map(str, args.mlp_dims))}_initialize_mode{args.initialize_mode}_lr_emb{args.lr_emb}_lr_nn{args.lr_nn}_l2_emb{args.l2_emb}_l2_nn{args.l2_nn}_batch_size{args.batch_size}'
...
args.load = './log/{}'.format(args.load)
...
trainer = Trainer(data_loader, data_config, model_config, train_config)
